...
export default class Calculator {
  ...
  public handleInput(inputData: string) {
    switch (inputData) {
      ...
      default: {
        const parseInputData = Number(inputData);

        if (Number.isNaN(parseInputData)) {
          throw new Error(`Invalid input: ${inputData}`);
        }

        this.display.concat(parseInputData.toString());
      }
    }
  }
  ...
  public executeOperation() {
    ...
    switch (this.#operator) {
      case "/":
        ...
        break;
      case "*":
        result = this.operation.multiply(this.#operand1, this.#operand2);
        break;
      case "-":
        result = this.operation.subtract(this.#operand1, this.#operand2);
        break;
      case "+":
